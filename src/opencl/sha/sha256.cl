...
(
    ...
    curl -sS https://raw.githubusercontent.com/hashcat/hashcat/master/OpenCL/inc_hash_functions.cl \
        | awk 'BEGIN{ good=0}  /define SHA256_S0_S/{good=1}  /SHIFT_RIGHT_64/{good=0}  {if (good > 0) print }'
    ...
    curl -sS https://raw.githubusercontent.com/hashcat/hashcat/master/OpenCL/inc_hash_constants.h \
        | awk 'BEGIN{ good=0}  /typedef enum sha2_32_constants/{good=1}  /typedef enum sha2_64_constants/{good=0}  {if (good > 0) print }'

    echo "// Snippet from inc_hash_sha256.cl"
    curl -sS https://raw.githubusercontent.com/hashcat/hashcat/master/OpenCL/inc_hash_sha256.cl \
        | awk 'BEGIN{ good=1}  /\/\/ sha256_hmac/{good=0}  {if (good > 0) print }'
) >> sha256.cl
*/
...
// Snippet from inc_hash_sha256.cl
...
void sha256_transform (const uint32_t *w0, const uint32_t *w1, const uint32_t *w2, const uint32_t *w3, uint32_t *digest)
{
  uint32_t a = digest[0];
  uint32_t b = digest[1];
  uint32_t c = digest[2];
...
void sha256_update_64 (sha256_ctx_t *ctx, uint32_t *w0, uint32_t *w1, uint32_t *w2, uint32_t *w3, const int len)
{
...
void sha256_update (sha256_ctx_t *ctx, const uint32_t *w, const int len)
{
  uint32_t w0[4];
  uint32_t w1[4];
  uint32_t w2[4];
...
void sha256_update_swap (sha256_ctx_t *ctx, const uint32_t *w, const int len)
{
  uint32_t w0[4];
  uint32_t w1[4];
  uint32_t w2[4];
...
void sha256_update_global (sha256_ctx_t *ctx, const __global uint32_t *w, const int len)
{
  uint32_t w0[4];
  uint32_t w1[4];
  uint32_t w2[4];
...
void sha256_update_global_swap (sha256_ctx_t *ctx, const __global uint32_t *w, const int len)
{
  uint32_t w0[4];
  uint32_t w1[4];
  uint32_t w2[4];
